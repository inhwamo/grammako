def check_sov_order(pos_tagged):
    has_subject = any(str(tag) in ['NP', 'NNG', 'NNP'] for _, tag in pos_tagged)  # Pronoun, Common Noun, Proper Noun
    has_object = any(str(tag) == 'JKO' for _, tag in pos_tagged)  # Object Case Particle: 을·를
    has_verb = any(str(tag).startswith('V') for _, tag in pos_tagged if isinstance(tag, str))  # Ensure tag is a string
...
def check_sentence_completeness(pos_tagged):
    if not any(str(tag).startswith('V') for _, tag in pos_tagged if isinstance(tag, str)):  # Ensure tag is a string
...
def check_proper_ending(pos_tagged): # sentence ends with ending POS or .?! e.g. "습니다"
    if str(pos_tagged[-1][1]) not in ['EF', 'EC','EP','SF']: 
        return "Sentence may be missing a proper ending." 
    return None 

def check_case_particles(pos_tagged):
    subject_particle = any(str(tag) in ['JKS', 'JX'] for _, tag in pos_tagged) # JKS = 이/가, JX = 은/는/도
    object_particle = any(str(tag) == 'JKO' for _, tag in pos_tagged) # 을·를
...
def check_honorific_consistency(pos_tagged):
    honorific_subject = any(str(word) in ['저', '제'] for word, _ in pos_tagged)
    honorific_verb = any(str(tag) == 'EP' for _, tag in pos_tagged) # EP = 았/었/
