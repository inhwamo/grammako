def check_sov_order(pos_tagged):
    ...
    has_object = any(tag == 'JKO' for _, tag in pos_tagged)
...
def check_sentence_completeness(pos_tagged):
    if not any(tag.startswith('V') for _, tag in pos_tagged):
        return "Sentence may be incomplete (missing a verb)."
    return None

def check_proper_ending(pos_tagged):
    if pos_tagged[-1][1] not in ['EF', 'SF']:
        return "Sentence may be missing a proper ending."
    return None

def check_case_particles(pos_tagged):
    subject_particle = any(tag in ['JKS', 'JX'] for _, tag in pos_tagged)
    object_particle = any(tag == 'JKO' for _, tag in pos_tagged)
...
def check_honorific_consistency(pos_tagged):
    ...
    honorific_verb = any(tag == 'EP' for _, tag in pos_tagged)
